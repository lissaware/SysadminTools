...
.DESCRIPTION
    This PowerShell script automates the mass testing process using Veeam Backup & Replication (VBR). 
    It creates and manages SureBackup jobs to facilitate efficient and scalable application testing in a virtual lab setup.
.NOTES  
    File Name  : vbr-dynamic-surebackup.ps1
...
$eMail                          = "Italerts@ncsfrac.onmicrosoft.com"
$VBRserver                      = "localhost"
...
$VbsStartOptions = New-VBRSureBackupStartupOptions -AllocatedMemory 100 -EnableVMHeartbeatCheck:$true -EnableVMPingCheck:$false -MaximumBootTime 1800 -ApplicationInitializationTimeout 600 -DisableWindowsFirewall:$true
...
if(!(Get-VBRApplicationGroup -Name $AppGroupName -ErrorAction Ignore)) {
    ...
    $VbrObjs = (Get-VBRBackupSession | Where-Object {$_.JobType -eq "Backup" -and $_.EndTime -ge $Date -and $_.Name -like "HDC*"}).GetTaskSessions() | Where-Object {$_.Status -eq "Success" -or $_.Status -eq "Warning" }
